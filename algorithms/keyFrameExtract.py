...
def smooth(x, window_len=13, window='hanning'):
    ...
    """
    print(f'提取视频长度:{len(x)}, 捕获window大小为:{window_len}')
    # if x.ndim != 1:
...
def getEffectiveFrame(videopath, st, ed, dir = None):
    # 如果文件目录不存在则创建目录
    ...
    # 使用 assert 检查帧范围是否有效
    assert st >= 0 and ed <= total_frames, f"Invalid frame range: {st} to {ed}"

    cap.set(cv2.CAP_PROP_POS_FRAMES, st)
    for cur in range(st, ed + 1):
